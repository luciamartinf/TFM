...
def ko_abundance(eggnog_sample, coverm_sample, samplename, rel_ko_abun:dict, sample_list, kos_dict, unit = units):
    ...
    # total_unit = globals()[f'total_{unit}']
    # #total_unit = f'total_{unit}'


    total_ko_abun = coverm_sample.total[unit]

    #total_ko_abun = coverm_sample.total_unit
    mapped_abun = 0
    ...
    for eggnog_row in eggnog_sample.rows:
        ...
        if eggnog_row.kegg_ko != '-':
            ...
            #eggnog_row.abundance = coverm_sample.rows[i].unit # comprobar que esto funciona!!
            eggnog_row.abundance = coverm_sample.rows[i].rpkm
