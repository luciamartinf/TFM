...
class Eggnog_sample(object):
    ...
    def load_sample(self, orf_dict, coverm_dict, og_dict, og_dict2, ko_dict2, kos_legend):
    ...
    def calc_contig_og_abundance(self, og, contig, TPM, des, king, og_dict2):
   
        # if the og is not in the global og_dict initialize it in the dictionary
        if og not in og_dict2.keys():
            og_dict2[og] = {}
            og_dict2[og]['description'] = des # save the og description 
            og_dict2[og]['kingdom'] = king # save the og kingdom
            for sample in Eggnog_sample.sample_list:
                og_dict2[og][sample] = 0 # initialize to 0 in all samples

        # check if the og is initialize in the sample's dictionary    
        if og not in self.contig_og.keys():
            self.contig_og[og] = {}
            self.contig_og[og]['description'] = des # save the og description 
            self.contig_og[og]['kingdom'] = king # save the og kingdom
            self.contig_og[og]['total'] = 0 # initialize total to 0

        # if the contig hasn't been sum
        if contig not in self.contig_og[og].keys():
            og_dict2[og][self.samplename] += float(TPM)  # add to global dict   
            
            self.contig_og[og][contig] = float(TPM)    # add contig abundance in sample dict
            self.contig_og[og]['total'] += float(TPM)  # add to sample dict

        return og_dict2
    

    def calc_contig_ko_abundance(self, contig, kos, TPM, ko_dict2, kos_legend):
        
        for ko in kos:
            if not ko in ko_dict2.keys():
                ko_dict2[ko] = {}
                try: 
                    ko_dict2[ko]['description'] = kos_legend[ko]['description'] # save the ko description 
                except:
            ...
            # check if the ko is initialize in the sample's dictionary    
            if ko not in self.contig_ko.keys():
                self.contig_ko[ko] = {}
                try: 
                    self.contig_ko[ko]['description'] = kos_legend[ko]['description'] # save the ko description 
                except:
                    self.contig_ko[ko]['description'] = '@'

                self.contig_ko[ko]['total'] = 0 # initialize total to 0

            # if the contig hasn't been sum
            if contig not in self.contig_ko[ko].keys():
                ko_dict2[ko][self.samplename] += float(TPM)  # add to global dict   
                
                self.contig_ko[ko][contig] = float(TPM)    # add contig abundance in sample dict
                self.contig_ko[ko]['total'] += float(TPM)  # add to sample dict
